    ...
    <section id="services" className="gateway-section" aria-label="بوابات خدمات ITL">
      <div className="container mx-auto px-4">
        ...
        <div className={`gateway-stage ${active.id}`} role="tabpanel">
          <div className="gateway-stage-copy"><div className="gateway-route-key"><span>01</span><i />سؤال يحدد المسار</div><div className="gateway-eyebrow"><ActiveIcon className="h-4 w-4" />{active.eyebrow}</div><h3>{active.title}</h3><p>{active.description}</p><Button asChild className="journey-primary-button group"><Link href="/#contact">{active.action}<ArrowLeft className="mr-2 h-4 w-4 transition-transform group-hover:-translate-x-1" /></Link></Button></div>
