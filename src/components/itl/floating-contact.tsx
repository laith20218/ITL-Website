...
export function FloatingContact() {
  ...
  return (
    <aside id="itl-floating-contact" className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-4 z-40 flex max-w-[calc(100vw-2rem)] flex-col items-end gap-2 sm:bottom-5 sm:left-5 sm:gap-3" aria-label="خيارات التواصل">
      <div id="itl-floating-contact-panel" className={`w-[min(18rem,calc(100vw-2rem))] origin-bottom-left overflow-hidden rounded-2xl border border-[#D4AF37]/35 bg-[#0B0B0B]/95 text-[#F3F0E8] shadow-[0_18px_60px_rgba(0,0,0,0.45)] backdrop-blur-xl transition-all duration-200 ease-out ${open ? 'pointer-events-auto translate-y-0 scale-100 opacity-100' : 'pointer-events-none translate-y-3 scale-95 opacity-0'}`} aria-hidden={!open}>
        ...
        <div className="space-y-1 p-2">
          <a href="/#contact" onClick={() => setOpen(false)} className="group flex min-h-11 items-center justify-between rounded-xl px-3 py-3 text-sm transition-colors hover:bg-[#D4AF37]/10">
          ...
          <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)} className="group flex min-h-11 items-center justify-between rounded-xl px-3 py-3 text-sm transition-colors hover:bg-[#D4AF37]/10">
            <span>عبر الواتساب</span><MessageCircle className="h-4 w-4 text-[#D4AF37] transition-transform group-hover:scale-110" />
          </a>
          <a href={gmailComposeUrl} target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)} className="group flex min-h-11 items-center justify-between rounded-xl px-3 py-3 text-sm transition-colors hover:bg-[#D4AF37]/10">
      ...
      <button type="button" onClick={() => setOpen((current) => !current)} aria-expanded={open} aria-controls="itl-floating-contact-panel" className="flex h-12 w-12 items-center justify-center rounded-full border border-[#E8C964]/70 bg-[#D4AF37] text-[#0A0A0A] shadow-[0_8px_30px_rgba(212,175,55,0.35)] transition-all duration-200 ease-out hover:-translate-y-0.5 hover:bg-[#E8C964] active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E8C964] focus-visible:ring-offset-2 focus-visible:ring-offset-black sm:h-14 sm:w-14">
        {open ? <ChevronDown className="h-5 w-5 sm:h-6 sm:w-6" aria-hidden="true" /> : <Send className="h-5 w-5 -rotate-45 sm:h-6 sm:w-6" aria-hidden="true" />}
