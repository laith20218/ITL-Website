...
function pdfPreviewUrl(value: string) {
  ...
  return driveId ? `https://drive.google.com/file/d/${encodeURIComponent(driveId)}/preview?embedded=true` : value
}
      ...
      {visibleFiles.length === 0 ? <div className="py-20 text-center text-muted-foreground">{kindMeta.empty}</div> : <div className="mx-auto grid max-w-5xl grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {visibleFiles.map((file, i) => {
          ...
          return <motion.div key={file.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="luxury-card rounded-xl p-5 transition-all hover:border-[#D4AF37]/40">
            ...
            {canPreview ? <Button type="button" onClick={() => { setViewerLoaded(false); setPreview(file) }} className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#D4AF37]/10 py-2 text-sm font-medium text-[#D4AF37] transition-colors hover:bg-[#D4AF37]/20"><ActionIcon className="h-4 w-4" />{cta}</Button> : <a href={file.fileUrl} download={file.kind === 'FILE'} target="_blank" rel="noopener noreferrer" className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#D4AF37]/10 py-2 text-sm font-medium text-[#D4AF37] transition-colors hover:bg-[#D4AF37]/20"><ActionIcon className="h-4 w-4" />{cta}</a>}
          </motion.div>
        })}
      </div>}

      <Dialog open={Boolean(preview)} onOpenChange={(open) => { if (!open) { setPreview(null); setViewerLoaded(false) } }}>
        <DialogContent dir="rtl" className="max-h-[94vh] max-w-[calc(100%-1rem)] border-[#D4AF37]/30 bg-[#0b0b0b] p-4 sm:max-w-5xl sm:p-6">
          {preview && <><DialogHeader className="pr-7 text-right sm:text-right"><DialogTitle className="text-[#E8C964]">{preview.title}</DialogTitle><DialogDescription>{previewPdf ? 'استعراض المستند داخل الموقع' : 'مشاهدة الفيديو داخل الموقع'}</DialogDescription></DialogHeader><div className="relative overflow-hidden rounded-lg border border-[#D4AF37]/20 bg-black">{!viewerLoaded && <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/80 px-6 text-center text-sm text-muted-foreground">جارٍ تحميل المعاينة… إذا منع المصدر التضمين، استخدم زر «فتح في نافذة جديدة» أدناه.</div>}{previewPdf ? <iframe src={pdfPreviewUrl(preview.fileUrl)} title={`معاينة ${preview.title}`} className="h-[68vh] min-h-[360px] w-full" allow="fullscreen" referrerPolicy="strict-origin-when-cross-origin" onLoad={() => setViewerLoaded(true)} /> : previewVideoId ? <div className="aspect-video"><iframe src={`https://www.youtube-nocookie.com/embed/${previewVideoId}?rel=0&playsinline=1`} title={`فيديو ${preview.title}`} className="h-full w-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; fullscreen" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen onLoad={() => setViewerLoaded(true)} /></div> : <div className="p-8 text-center text-sm text-muted-foreground">تعذر تحضير معاينة هذا الرابط.</div>}</div><DialogFooter className="mt-1 gap-2 sm:justify-start"><a href={preview.fileUrl} target="_blank" rel="noopener noreferrer" className="inline-flex h-9 items-center justify-center rounded-md border border-[#D4AF37]/30 px-3 text-sm text-[#E8C964] transition-colors hover:bg-[#D4AF37]/10"><ExternalLink className="ml-1 h-4 w-4" />فتح في نافذة جديدة</a><DialogClose asChild><Button type="button" variant="ghost">إغلاق</Button></DialogClose></DialogFooter></>}
        </DialogContent>
      </Dialog>
    </div>
  )
